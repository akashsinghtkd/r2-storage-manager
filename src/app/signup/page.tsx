...
export default function SignupPage() {
  ...
  const strengthColor = ["", "text-destructive", "text-yellow-500", "text-green-500"];
  const strengthBg = ["", "bg-destructive", "bg-yellow-500", "bg-green-500"];
    ...
    <div className="min-h-screen flex bg-background">
      ...
      <div className="flex-1 flex items-center justify-center p-6 sm:p-10 lg:p-16">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: [0.22, 1, 0.36, 1] }} className="w-full max-w-[400px]">
          ...
          <div className="mb-8">
            <h2 className="text-2xl font-extrabold tracking-tight mb-2">Create your account</h2>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input id="signup-password" type={showPassword ? "text" : "password"} value={password}
                  ...
                  required minLength={6} className="pl-10 pr-12 h-12" />
              ...
              {password.length > 0 && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="flex items-center gap-3 pt-1">
                  <div className="flex gap-1 flex-1">
                    {[1, 2, 3].map((l) => (
                      <div key={l} className={`h-1 flex-1 rounded-full transition-colors ${strength >= l ? strengthBg[strength] : "bg-border"}`} />
                    ))}
                  </div>
                  <span className={`text-xs font-bold ${strengthColor[strength]}`}>{strengthLabel[strength]}</span>
                </motion.div>
              )}
            </div>
